...
const STEPS = 500n; // 500 price steps
const PRICE_INCREASE_RATE = 1015n; // 1.5% increase per step
const TARGET_LOCKED_RATIO = 100n; // targeting 1% of base asset supply
...
function calculateInitialPrice(
  ...
  steps: bigint = STEPS,
  priceIncreaseRate: bigint = PRICE_INCREASE_RATE,
  targetLockedRatio: bigint = TARGET_LOCKED_RATIO
): bigint {
...
export async function getDefaultBondParamsForBaseAsset(
  baseAsset: `0x${string}`
) {
  ...
  const supplyPerStep = CHILD_TOKEN_MAX_SUPPLY / STEPS;
  const stepRanges = Array.from(
    { length: Number(STEPS) },
    (_, i) => BigInt(i + 1) * supplyPerStep
  );
  const stepPrices = Array.from({ length: Number(STEPS) }, (_, i) => {
